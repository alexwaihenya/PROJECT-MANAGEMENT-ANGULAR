...
import { Data, User, userCustom, userDetails } from "../Interfaces/userCustom";
...
export const getAllUsers = async(req:userCustom,res:Response)=>{
    try {
        ...
        const pool = await mssql.connect(sqlConfig);
        const results = (await pool.request().query("select * from Users")).recordset;
        if (results.length == 0) {
...
export const pendingUsers = async(req:userCustom, res:Response)=>{
    try {
        ...
        const users = await(
            await pool.request().execute('UsersWithNoTasks')).recordset
...
export const updateComplete = async (req:projectCustom, res:Response)=>{
    try {
        ...
        await pool.request()
        .input('id', mssql.VarChar, project_id)
        .execute('setComplete')
